...
?>
  ...
  <div class="content bg-<?php echo $bodybackground; ?>">
    ...
    <?php 
    $template_setting_id = 3;
    $sql = "SELECT `Page4_TextHead1` FROM `template_setting` WHERE id = " . $template_setting_id;
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $Page4_TextHead1 = $result['Page4_TextHead1']; 

    $select_stmt_26 = $conn->prepare("SELECT `Details`, `connect`, `show` FROM `text` WHERE id = " . $Page4_TextHead1);
    $select_stmt_26->execute();
    $row_26 = $select_stmt_26->fetch(PDO::FETCH_ASSOC);
    $Details_26 = $row_26['Details'];
    $connect_26 = $row_26['connect'];
    $show_26 = $row_26['show'];

    // ตรวจสอบค่าในฟิลด์ "show"
    if ($show_26 === 'on') { // เมื่อค่าในฟิลด์ "show" เป็น "on" เท่านั้น
    ?>
              
          <h2><?php echo $Details_26; ?></h2>
          <?php   } // ปิดเงื่อนไข if?>
          <div class="clr"></div>
              <?php 
    $template_setting_id = 3;
    $sql = "SELECT `Page4_Textcontent1` FROM `template_setting` WHERE id = " . $template_setting_id;
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $Page4_Textcontent1 = $result['Page4_Textcontent1']; 

    $select_stmt_27 = $conn->prepare("SELECT `Details`, `connect`, `show` FROM `text` WHERE id = " . $Page4_Textcontent1);
    $select_stmt_27->execute();
    $row_27 = $select_stmt_27->fetch(PDO::FETCH_ASSOC);
    $Details_27 = $row_27['Details'];
    $connect_27 = $row_27['connect'];
    $show_27 = $row_27['show'];

    // ตรวจสอบค่าในฟิลด์ "show"
    if ($show_27=== 'on') { // เมื่อค่าในฟิลด์ "show" เป็น "on" เท่านั้น
    ?>
          <p><?php echo $Details_27; ?></p>
          <?php   } // ปิดเงื่อนไข if?>
        </div>
        <div class="article">
<?php 
  $template_setting_id = 3;
  $sql = "SELECT `Page4_TextHead2` FROM `template_setting` WHERE id = " . $template_setting_id;
  $stmt = $conn->prepare($sql);
  $stmt->execute();
  $result = $stmt->fetch(PDO::FETCH_ASSOC);
  $Page4_TextHead2 = $result['Page4_TextHead2']; 

$select_stmt_28 = $conn->prepare("SELECT `Details`, `connect`, `show` FROM `text` WHERE id = " . $Page4_TextHead2);
$select_stmt_28->execute();
$row_28 = $select_stmt_28->fetch(PDO::FETCH_ASSOC);
$Details_28 = $row_28['Details'];
$connect_28 = $row_28['connect'];
$show_28 = $row_28['show'];

if ($show_28=== 'on') { // เมื่อค่าในฟิลด์ "show" เป็น "on" เท่านั้น
?>
          <h2><?php echo $Details_28; ?></h2>
            <?php   } // ปิดเงื่อนไข if?>
          <div class="clr"></div>
<?php 
// ดึงข้อมูล "id=3" จากตาราง button
$select_stmt_show14 = $conn->prepare("SELECT `show`,`format` FROM `related` WHERE id = 14");
$select_stmt_show14->execute();
$row_show14 = $select_stmt_show14->fetch(PDO::FETCH_ASSOC);
$format_show14 = $row_show14['format'];
$show_show14 = $row_show14['show'];
                            
// ตรวจสอบค่าในฟิลด์ "show"
if ($show_show14 === 'on') { // เมื่อค่าในฟิลด์ "show" เป็น "on" เท่านั้น
?>
          <form action="#" method="post" id="sendemail">
            <ol>
              ...
              <li>
                <button type="image" name="imageField" id="imageField"  class="send" >Submit</button>
                <div class="clr"></div>
              </li>
            </ol>
          </form>
