<?php 
    ...
    if (isset($_REQUEST['delete_id'])) {
        ...
        $delete_stmt = $conn->prepare('DELETE FROM `tabmenu` WHERE id = :id');
        $delete_stmt->bindParam(':id', $id);
        $delete_stmt->execute();

        header('Location:Dev_tabmenu.php');
    }
?>
<!DOCTYPE html>
<html lang="en">
...
<div class="wrapper">
    ...
    <div class="header">TAB MENU</div>
    <div class="info">
    <div class="container">
                    <div class="display-3 text-center"></div>
                    <a href="Dev_tabmenuadd.php" class="btn btn-dark mb-3">Add+</a>
                    <table class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
            <?php 
                $select_stmt = $conn->prepare("SELECT * FROM `tabmenu`");
                $select_stmt->execute();
            ...
            ?>
                            ...
                            <tr>
                                ...
                                <td><?php echo $row["subtopic1"]; ?></td>
                                <td><?php echo $row["subtopic2"]; ?></td>
                                <td><?php echo $row["subtopic3"]; ?></td>
                                <td><?php echo $row["subtopic4"]; ?></td>
                                <td><?php echo $row["subtopic5"]; ?></td>
                                <td style="color: <?php echo $row["show"] === 'on' ? 'green' : 'red'; ?>"><?php echo $row["show"]; ?></td>
                                <td><a href="Dev_tabmenuedit.php?update_id=<?php echo $row["id"]; ?>"
