...
<?php   } // ปิดเงื่อนไข if?>
    ...
    <?php 
    $template_setting_id = 2;
   $sql = "SELECT `page8_content` FROM `template_setting` WHERE id = " . $template_setting_id;
    $stmt = $conn->prepare($sql);
     $stmt->execute();
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $page8_content = $result['page8_content'];
    
$select_stmt_content = $conn->prepare("SELECT colors, `tcolors`, `border`, `radius`, `bcolors` FROM `colors` WHERE id = " . $page8_content);
...
?>
	...
	<?php 
    // ดึงข้อมูล "id=3" จากตาราง button
    $select_stmt_23 = $conn->prepare("SELECT `Details`, `connect`, `show` FROM `text` WHERE id = 23");
