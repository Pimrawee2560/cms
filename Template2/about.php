...
<main>
    ...
    <?php 
      $sql = "SELECT `page2_text_head1` FROM `template_setting` WHERE id = 1";
      $stmt = $conn->prepare($sql);
      $stmt->execute();
      $result = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    <?php   } // ปิดเงื่อนไข if?>
    <?php 
    $sql = "SELECT `page2_text_content1` FROM `template_setting` WHERE id = 1";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    <?php   } // ปิดเงื่อนไข if?>
                    <div class="about-details-cap mb-50">
    <?php 
    $sql = "SELECT `page2_text_head2` FROM `template_setting` WHERE id = 1";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    <?php echo $Details_11; ?>
        <?php   } // ปิดเงื่อนไข if?>
       <?php 
      $sql = "SELECT `page2_text_content2` FROM `template_setting` WHERE id = 1";
      $stmt = $conn->prepare($sql);
      $stmt->execute();
      $result = $stmt->fetch(PDO::FETCH_ASSOC);
