...
$select_stmt_title = $conn->prepare("SELECT `name` FROM `title` WHERE id = 10");
...
<!--===============================================================================================-->
<?php 
$select_stmt_title = $conn->prepare("SELECT `image`, `show` FROM `title` WHERE id = 10");
$select_stmt_title->execute();
$row_title = $select_stmt_title->fetch(PDO::FETCH_ASSOC);
...
<?php   } // ปิดเงื่อนไข if?>
	...
	<?php 
$template_setting_id = 2;
$sql = "SELECT `page10_content` FROM `template_setting` WHERE id = " . $template_setting_id;
$stmt = $conn->prepare($sql);
$stmt->execute();
$result = $stmt->fetch(PDO::FETCH_ASSOC);
$page10_content = $result['page10_content'];

$select_stmt_content = $conn->prepare("SELECT colors, `tcolors`, `border`, `radius`, `bcolors` FROM `colors` WHERE id = " . $page10_content);
...
<?php 
// ดึงข้อมูล "id=3" จากตาราง button
$select_stmt_showone = $conn->prepare("SELECT `show` FROM `related` WHERE id = 11");
				...
				<?php
                 try {
					...
					if ($stmt->rowCount() > 0) {
						// วนลูปผลลัพธ์และแสดงข้อมูลในรูปแบบ HTML
						while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
							echo '<div class="p-b-23">';
							echo '<div class="how2 how2-cl4 flex-s-c">';
							echo '<h3 class="f1-m-2 cl3 tab01-title">' . $row["heading"] . '</h3>';
							echo '</div>';
							echo '<ul class="p-t-35">';
							
							// แสดง subtopic1
							echo '<li class="flex-wr-sb-s p-b-22">';
							echo '<div class="size-a-8 flex-c-c borad-3 size-a-8 bg9 f1-m-4 cl0 m-b-6">1</div>';
							echo '<a href="' . $row["connect1"] . '" class="size-w-3 f1-s-7 cl3 hov-cl10 trans-03">' . $row["subtopic1"] . '</a>';
							echo '</li>'; // ปิด tag li ที่ subtopic1
				
							// ตรวจสอบและแสดง subtopic2 ถ้ามี
							if (!empty($row["subtopic2"])) {
								echo '<li class="flex-wr-sb-s p-b-22">';
								echo '<div class="size-a-8 flex-c-c borad-3 size-a-8 bg9 f1-m-4 cl0 m-b-6">2</div>';
								echo '<a href="' . $row["connect2"] . '" class="size-w-3 f1-s-7 cl3 hov-cl10 trans-03">' . $row["subtopic2"] . '</a>';
								echo '</li>'; // ปิด tag li ที่ subtopic2
							}
				
							// ตรวจสอบและแสดง subtopic3 ถ้ามี
							if (!empty($row["subtopic3"])) {
								echo '<li class="flex-wr-sb-s p-b-22">';
								echo '<div class="size-a-8 flex-c-c borad-3 size-a-8 bg9 f1-m-4 cl0 m-b-6">3</div>';
								echo '<a href="' . $row["connect3"] . '" class="size-w-3 f1-s-7 cl3 hov-cl10 trans-03">' . $row["subtopic3"] . '</a>';
								echo '</li>'; // ปิด tag li ที่ subtopic3
							}
							if (!empty($row["subtopic4"])) {
								echo '<li class="flex-wr-sb-s p-b-22">';
								echo '<div class="size-a-8 flex-c-c borad-3 size-a-8 bg9 f1-m-4 cl0 m-b-6">4</div>';
								echo '<a href="' . $row["connect4"] . '" class="size-w-3 f1-s-7 cl3 hov-cl10 trans-03">' . $row["subtopic4"] . '</a>';
								echo '</li>'; // ปิด tag li ที่ subtopic3
							}
							if (!empty($row["subtopic5"])) {
								echo '<li class="flex-wr-sb-s p-b-22">';
								echo '<div class="size-a-8 flex-c-c borad-3 size-a-8 bg9 f1-m-4 cl0 m-b-6">5</div>';
								echo '<a href="' . $row["connect5"] . '" class="size-w-3 f1-s-7 cl3 hov-cl10 trans-03">' . $row["subtopic5"] . '</a>';
								echo '</li>'; // ปิด tag li ที่ subtopic3
							}
